...
6. **EBS VOD 저장 버튼** (`ebs-downloader.js`) — EBS VOD 재생 페이지에
   "⬇ 영상 저장" 버튼을 띄운다. 페이지에 들어있는 화질 중 **최고화질**을 골라
   브라우저 다운로드로 저장 (파일명은 영상 제목).
   구독이 필요한 콘텐츠는 서버가 60초 맛보기만 보내주므로 저장되지 않는다.
   개인 소장용으로만 사용할 것 (재배포는 저작권 침해).
